...
set -x _tabry_rs_import_path "$_tabry_rs_import_path:~/.local/share/tabry"
...
function __fish_tabry_internal_invoke
  ...
  # todo: here
  set -x TABRY_IMPORT_PATH $_tabry_rs_imports_path
  set result ($_tabry_rs_executable "$cmd" "$cursor_position")
  echo $result
end

# return true if tabry only reports files
